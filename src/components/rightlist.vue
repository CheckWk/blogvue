  ...
  <div class="rightlistBox">
    <section>
      <div class="r1-head">
        <img
          :src="
            this.$store.state.themeObj.right_smailimg
              ? this.$store.state.themeObj.right_smailimg
        ...
        />
        <h1 >
          <span>{{ this.$store.state.themeObj.right_user_start
              ? this.$store.state.themeObj.right_user_start
              : '请别再呼唤我为孤独之人'}}</span>
        </h1>
      </div>
      <div class="r1-body">
        <p>{{ this.$store.state.themeObj.right_autograph
              ? this.$store.state.themeObj.right_autograph
              : 'hello word!'}}</p>
        <div class="catch-me">
          <div class="">
            <el-tooltip class="item" v-show="catchMeObj.git" content="Github" placement="top">
              <a :href="catchMeObj.git" target="_blank"
                ><i class="fa fa-fw fa-github"></i
              ></a>
            </el-tooltip>
            <el-tooltip class="item" v-show="catchMeObj.qq" effect="dark" content="QQ" placement="top">
...
export default {
  ...
  created() {
    ...
    //初始化右侧个人信息数据
    this.catchMeObj=this.$store.state.rightInfoData
    //查询浏览量最多的10篇文章数据
