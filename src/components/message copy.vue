...
<img
            class="avater-img"
            :src="commentData.avatar?commentData.avatar:$store.state.errorImg"
            :onerror="commentData.avatar?commentData.avatar:$store.state.errorImg"
            alt="#"
          />
          <!-- <el-tag type="success">标签二</el-tag> -->
          <p>{{commentData.author?commentData.author:'访客'}}</p>
          </div>
          ...
          <div class="input-right-box">
            ...
            <div class="btn-box">
              <el-form-item class="info-input" >
                <el-input placeholder="QQ"  @blur="changeInfo" size="small" v-model="commentData.qq"></el-input>
              </el-form-item>
              <el-form-item class="info-input">
                <el-input
                  type="email"
              ...
              <el-form-item class="info-input">
                <el-input type="text" @blur="changeInfo" placeholder="网站" size="small" v-model="commentData.url"></el-input>
              </el-form-item>
            </div>
          </div>
        </div>
            ...
            <article class="">
              <header>
                <img
                  ...
                  :onerror="item.avatar?item.avatar:$store.state.errorImg"
                />
                <div class="i-name">
            
                  <a :href="item.url?item.url:'###'" :style="{pointerEvents:item.url? 'all':'none'}">{{ item.author?item.author:'访客' }}</a>
                </div>
                <!-- <div class="i-class">
                                    {{item.label}}
                                </div> -->
              ...
              <multistage :children="item.children"></multistage>
                ...
                <article class="">
                  <header>
                    <img
                      ...
                      :onerror="citem.avatar?citem.avatar:$store.state.errorImg"
                    />
                    <div class="i-name">
                      <a :href="citem.url?citem.url:'###'" :style="{pointerEvents:citem.url? 'all':'none'}">{{ citem.author?citem.author:'访客' }}</a>
                       <span>回复</span>
                      <a :href="item.url?item.url:'###'" :style="{pointerEvents:item.url? 'all':'none'}">{{ citem.toCommentUserName?citem.toCommentUserName:'访客' }}</a>
...
<script>
import multistage from "@/components/commentList.vue";
// import multistage from "";
// const multistage = () =>  import( "../components/commentList.vue");
import {
  sendComment,
  getArticleComment,
  getLinkComment,
...
export default {
   components: {
     //定义组件
            // multistage:require('@/components/commentList.vue').default,
            multistage,
        },
  data() {
    //选项 / 数据
  ...
  methods: {
    ...
    choseEmoji: function (inner) {
      this.commentData.content += "[" + inner + "]";
    },
    ...
    respondMsg: function (rootId, toCommentId, toCommentUserId) {
      ...
      if (true) {
        ...
        this.commentData.rootId = rootId;
        this.commentData.toCommentId = toCommentId;
        this.commentData.toCommentUserId = toCommentUserId;
    ...
    changeInfo:function(e){
      ...
      if(qq){
        this.commentData.avatar="https://q.qlogo.cn/g?b=qq&nk="+qq+"&s=100"
        //获取昵称
        getQQInfo(qq).then((res)=>{
          console.log(res)
          this.commentData.author=res.name;
        })
...
.comment-from .comment-input {
  display: flex;
}
.comment-from .comment-input .avater-img {
  padding: 20px;
  margin-right: 0;
}
.comment-from .comment-input .input-left-box {
  /* flex-wrap: nowrap; */
 display: flex;
...
.comment-from .comment-input .input-right-box {
  /* flex-wrap: nowrap; */
 
  width: 80%;
}

.comment-from .comment-input .input-right-box .top-box {
}
.comment-from .comment-input .input-right-box .btn-box {
   display: flex;
   justify-content: space-between;
}

.comment-from .comment-input .info-input {
  /* float: left; */
